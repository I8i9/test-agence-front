...
const CreateContratStep4 = ({ setContratData, contratData, next, prev, totalAmount = 0, hasConducteurSupplementaire , doublePrev }) => {

    const mustDoublePrev = !contratData?.has_conducteur && hasConducteurSupplementaire;
    console.log("demande dans CreateContratStep5:", !contratData?.has_conducteur , hasConducteurSupplementaire, !contratData?.has_conducteur && hasConducteurSupplementaire);
  console.log("Contrat Data at Step 4:", contratData)
  
  const [selectedOption, setSelectedOption] = useState(contratData?.option_paiement || "full") 
  ...
  const formSchema = z.object({
    montant_paye: z.union([
      ...
      .refine(val => val <= totalAmount, `Le montant payé ne peut pas dépasser ${totalAmount.toFixed(2)} DT`), 
  ...
  const form = useForm({
    ...
    defaultValues: {
      montant_paye: contratData?.montant_paye || 0,
      mode_paiement: contratData?.mode_paiement || "ESPECE",
      reference_paiement: contratData?.reference_paiement || "",
      paiement_echelonne: contratData?.paiement_echelonne || false,
    },
  })
  ...
  useEffect(() => {
    if (selectedOption === "full") {
      form.setValue("montant_paye", totalAmount)
      form.setValue("paiement_echelonne", false)
    } else if (selectedOption === "partial") {
      form.setValue("montant_paye", contratData?.montant_paye || 0)
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <form id="declarer-depense-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6"> 
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                {/* Full Payment Option */}
                <Card
                  className={`flex-1 p-4 cursor-pointer shadow-none transition-all border-2 ${
                    selectedOption === "full"
                      ? "border-green-500 "
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <X
                        className={`w-5 h-5 ${selectedOption === "none" ? "text-white" : "text-red-500"}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      {form.formState.errors?.montant_paye ? (
                        ...
                        <FormDescription>
                          {selectedOption === "partial" && `Max: ${totalAmount.toFixed(2)} DT`}
      ...
      <div className="flex justify-between mt-4 pt-4 border-t">
        <Button variant="outline" onClick={mustDoublePrev ? doublePrev : prev} className="rounded-sm">
...
export default CreateContratStep4
