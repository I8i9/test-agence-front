...
import { useDebounce } from '../../../../utils/useDebounce';
   ...
   nationalite: z.string({required_error: "La nationalité est requise lorsque le passeport est utilisé",invalid_type_error: "La nationalité doit être une chaîne de caractères"}).regex(/^[A-Za-z\s]+$/, "La nationalité doit contenir uniquement des lettres.").optional(),
    ...
    const Conducteur = ({supp , setContratSpData, ContratSpData, next, prev }) => {

    const form = useForm({
        resolver: zodResolver(formSchema),
        shouldUnregister: true,
        defaultValues: {
        nom_prenom: !supp ? ContratSpData.conducteur?.nom_prenom || '' : ContratSpData.conducteur_supp?.nom_prenom || '',
        date_naissance: !supp ? ContratSpData.conducteur?.date_naissance || undefined : ContratSpData.conducteur_supp?.date_naissance || undefined,
        nationalite:  '',
        cin_passport: !supp ? ContratSpData.conducteur?.cin_passport || '' : ContratSpData.conducteur_supp?.cin_passport || '',
        date_delivrance_cin:  undefined,
        permis_conduire: !supp ? ContratSpData.conducteur?.permis_conduire || '' : ContratSpData.conducteur_supp?.permis_conduire || '',
        date_delivrance_permis: !supp ? ContratSpData.conducteur?.date_delivrance_permis || undefined : ContratSpData.conducteur_supp?.date_delivrance_permis || undefined,
        adresse: !supp ? ContratSpData.conducteur?.adresse || '' : ContratSpData.conducteur_supp?.adresse || '',
        telephone: !supp ? ContratSpData.conducteur?.telephone || '' : ContratSpData.conducteur_supp?.telephone || '',
        },
    });
    ...
    const onSubmit = (data) => {
        ...
        date_delivrance_cin: data?.date_delivrance_cin ||null,
    ...
    const handleSkipStep = () => {
        if (supp) {
        setContratSpData(prev => ({ ...prev, conducteur_supp: null }));
        } else {
            setContratSpData(prev => ({ ...prev, conducteur: null }));
        }
        next();
    }
    ...
    return (
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-y-hidden space-y-4 p-1">
            <h3 className="flex items-center gap-1 font-semibold text-lg">
                {supp ? "Conducteur Supplémentaire" : "Conducteur Principal"}
            </h3>
        ...
        <div className="flex justify-between pt-4 mt-4 border-t">
            ...
            >
                {supp ? "Ignorer conducteur supp." : "Ignorer conducteur principal"}
            </Button>
