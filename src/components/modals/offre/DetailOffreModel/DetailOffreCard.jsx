...
import {  MousePointerClick, CalendarCheck2, MapPin,Info, Handshake, MinusCircle, PlusCircle, CalendarX2 } from 'lucide-react';
    ...
    <Card className="shadow-none h-auto">
      ...
      <CardContent className="grid grid-cols-2 gap-x-8 gap-y-5">
          ...
          <DetailItem icon={CalendarCheck2} label="Date début">
            <ToolTipCustom
            trigger={<p className=" max-w-fit  truncate">{formatDateDDMMYYYY(data.date_debut_offre)}</p>}
            message={FormatDateEEEEddMMyyyy(data.date_debut_offre)}
            />
          </DetailItem>

          <DetailItem icon={CalendarX2} label="Date fin">
            <ToolTipCustom
              trigger={<p className=" max-w-fit  truncate">{formatDateDDMMYYYY(data.date_fin_offre)}</p>}
              message={FormatDateEEEEddMMyyyy(data.date_fin_offre)}
            />
          </DetailItem>

      {data.clics_offre !== undefined && (
            <DetailItem icon={MousePointerClick} label="Nombre de clics">
              {convertNumberToK(data.clics_offre)}
            </DetailItem>
          )}
          <DetailItem icon={Handshake} label='Total de Demandes'>
            {data.nombres_demandes}
          </DetailItem>
