...
import { DollarSign, Info, CreditCard } from 'lucide-react';
    ...
    <Card className={`shadow-none max-h-[300px]`}>
      ...
      <CardContent className="relative space-y-4">
        <div className="grid grid-cols-2 gap-x-4 gap-y-8">
          ...
          <DetailItem icon={DollarSign} label="Caution">
            {data.depot_pol} DT
          </DetailItem>
        </div>
          ...
          <div className="flex flex-col w-full gap-2">
            <div className={`w-full ${showMore ? 'shadow-sm rounded-md bg-white' : ''}`}>
              <Table className="w-full">
                ...
                <TableBody className={`${ranges.length > 2 ? 'max-h-[210px] overflow-y-auto' : ''}`}>
                  {ranges.length === 0 ? (
                    <TableRow>
                    ...
                    <>
                      {ranges.slice(0, showMore ? ranges.length : 2).map(({ min, max, price }, i) => {
                        const label = max === null ? `+${min} jours` : `${min} - ${max} jours`;
                        const hasDiscount = !!data?.promo?.taux_promo;
                        const finalPrice = hasDiscount ? applyDiscount(price) : price;

                        return (
                          <TableRow key={i} className='h-[10px]'>
                      ...
                      {ranges.length > 2 && (
                        <TableRow className='rounded-b-md'>
                          <TableCell
                            colSpan={2}
                            onClick={toggleShowMore}
                            className="text-sm font-medium text-primary text-center cursor-pointer"
                          >
