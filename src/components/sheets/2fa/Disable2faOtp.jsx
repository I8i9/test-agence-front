...
import { useDisableVerifyOtp2fa } from "../../../api/queries/2fa/useVerifyDisableOtp";




const Disable2faOtp = (props) => {
    ...
    const OTPFormSchema = z.object({
        ...
        const { mutate: verifyOtp, isPending } = useDisableVerifyOtp2fa();

        const { mutate: getOtp, isPending : isGettingOtp} = useGet2faDisableOtp({enabled : !blocked});
        

        const HandleSubmit = (data) => {
            verifyOtp({ otp: data.otp }, {
                onSuccess: () => {
                    props.setCurrentForm('disableFinish');
                    setNumberOfTries(1);
                },
                onError: () => {
        ...
        const handleResend = () => {
          getOtp({password_agent : props?.password }
            , {
                onSuccess: () => {
