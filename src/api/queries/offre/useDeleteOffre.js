...
const deleteOffreApi = async (id) => {
    const response = await mainapi.delete(`/offre/${id}`);
    return response.data;
}

export function useDeleteOffre({ onSuccess: onSuccessCallback, onError: onErrorCallback } = {}) {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: deleteOffreApi,
        onError: (error) => {
            if(error.response) {
        ...
        onSuccess: (_, variables) => {
            ...
            toast.success('La Offre a été désactivée avec succès.');
